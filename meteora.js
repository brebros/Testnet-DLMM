...
// ── Auto refresh Meteora pool data tiap 5 menit ──
// Pool data di-pass ke position object → startFeeAccumulation pakai data ini
...
function startRealFeeTracking(positions, onUpdate) {
  ...
  // Langsung update pool data sekali (non-blocking)
  updatePositionFeesWithRealData(positions).then(() => {
...
// ── Get pool data untuk satu pair (expose ke app.js) ──
async function getPoolDataForPair(pairId) {
  const pair = PAIRS.find(p => p.id === pairId);
  if (!pair) return null;
  return await fetchMeteoraPools(pair.base, pair.quote);
}
